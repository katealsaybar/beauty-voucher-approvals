 ...
 {id:'welcome',name:'Welcome Pack',short:'Welcome Pack',sub:'post-purchase · starts on voucher:paid',
  ...
  nodes:[
   ...
   /* The 8 Pillars Journal was built, shipped and then delivered by nothing at all: it is
      promised in the All-In VIP Year inclusions and appeared in none of the emails and
      none of the workflow. Email 1b closes the second half of that, written 21 Aug.
      This pair is where it belongs, a few hours after purchase,
   ...
   {t:'email',title:'Email 1b · your 8 Pillars Journal',sub:'VIP only · +3 hours',emailKey:'pillars',flag:'WRITTEN 21 AUG',flagOk:true,tip:{ch:'Email',sub:'Your 8 Pillars Journal',flag:'WRITTEN 21 AUG',flagOk:true,
     note:'WRITTEN 21 AUG, the fifteenth email and the only one that goes to a single tier. It carries one link, <code>{{journal_url}}</code>, and that URL does not exist yet: <code>eight-pillars/eight-pillars.html</code> is a client-facing page sitting in this pack, which is not a client-facing place. It has to be hosted before this can send. The preview button below points at the local file so the copy can be approved now.',
     rows:[['Sends to','<code>tier:vip</code> only'],['Timing','+3 hours after Email 1'],['Carries','the journal link, nothing else to do'],['Not gated on','the Confidence Mapping'],['Still needed','a hosted URL for <code>{{journal_url}}</code>'],['For Tara','yes or no on the founder quote']],
     long:'Deliberately not gated on the mapping. The Home Ritual Kit is withheld until she is mapped and that is real policy, but the journal is not part of the kit: the VIP inclusions list it separately, beside the Reset Journal. Gating it would mean the highest-paying tier waits on a quiz for the one asset that is hers alone. Three hours rather than the same minute, so it does not arrive as a third message alongside the confirmation email and the WhatsApp. The page saves her answers to her own device and prints to PDF; nothing comes back to us, so there is no capture step to build behind it. <b>The copy never says "only".</b> In Tara\'s Notion the 8 Pillars Self-Assessment is recorded at Offer Level: lead magnet, described as a free monthly self-assessment, with a website and affiliate tie-in planned; in this pack it is a VIP inclusion on an AED 4,500 tier. Those two futures are not reconciled and the call is Tara\'s. So the email states what she received, which stays true either way, and makes no exclusivity claim, which is the only line that would break if the journal ever goes free. Do not add "exclusive" or "no other tier" to the template.'}},
