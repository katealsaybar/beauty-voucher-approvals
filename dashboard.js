...
(function () {
  ...
  var filter = 'todo';   // todo | all | notes | suggestions | done
  var query = '';
  ...
  function threads() {
    var byId = {}, roots = [];
    notes.forEach(function (n) { byId[n.id] = n; n.replies = []; });
  ...
  function render() {
    ...
    if (!n.length && !s.length) {
      outEl.innerHTML = '<div class="empty">' +
        (query ? 'Nothing matches that search.' : 'Nothing here yet. Notes and suggestions left on the pack or the automations window show up here.') +
        '</div>';
      return;
    }
    ...
    Array.prototype.forEach.call(outEl.querySelectorAll('[data-arch-note]'), function (b) {
      b.addEventListener('click', function () { archive('approval_notes', b.dataset.archNote, { archived: true }); });
    });
    Array.prototype.forEach.call(outEl.querySelectorAll('[data-arch-sugg]'), function (b) {
      b.addEventListener('click', function () { archive('approval_suggestions', b.dataset.archSugg, { status: 'archived' }); });
    });
  }

  function archive(table, id, patch) {
    db.from(table).update(patch).eq('id', id).then(function (res) {
      if (res.error) { alert('Could not archive that: ' + res.error.message); return; }
      load();
    });
  }
  ...
  function load() {
    Promise.all([
      db.from('approval_notes').select('*').eq('pack_id', PACK_ID).eq('archived', false)
      ...
      if (err) {
        outEl.innerHTML = '<div class="err"><strong>Could not load.</strong><br>' + esc(err.message) +
          '<br><br>If this says a table is missing, run notes_setup.sql in Supabase first.</div>';
        return;
      }
      notes = res[0].data || [];
